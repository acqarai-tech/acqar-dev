...
export default function CommentSection({ areaName, areaId, seedComments, className = '' }) {
  ...
  // Persists to the real area_comments table when we have an areaId (any
  // DB-linked area). Optimistic UI update happens either way so posting
  // still feels instant, and still degrades gracefully to local-only for
  // areas with no area_id (pure synthetic preview reports) or if the
  // insert fails — never blocks the person from posting.
  const handlePost = async () => {
    const text = draft.trim()
    if (!text) return
    const localComment = { id: Date.now(), name: 'You', initials: 'Y', hoursAgo: 0, text, score: 1, myVote: 'up' }
