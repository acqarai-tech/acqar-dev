...
async function fetchPastTabData(areaId, areaName, keyDevelopers, zoneType, row) {
  ...
  // Area maturity — built from real dld_projects + area_intelligence
  // fields already available, rather than the old 4-row generic stub.
  // NOTE: year-established, master-developer, total-area, parks, and
  // retail-sqft have no source table yet (they're static area metadata,
  // not transaction data) — those still need a dedicated `area_metadata`
  // table to reach full JVC-style parity; this fetcher can't invent them.
  const projRows = dldProjects.data || []
  if (projRows.length || row) {
    const active = projRows.filter((p) => p.project_status === 'ACTIVE')
    ...
    const maturity = [{ label: 'Zone', value: zoneType || (row?.verdict === 'buy' ? 'Growth corridor' : 'Established') }]
    if (avgCompletion != null) maturity.push({ label: 'Completion rate', value: `~${avgCompletion}% built` })
    maturity.push({ label: 'Active off-plan projects', value: String(row?.active_project_count ?? active.length) })
    if (pipelineUnits) maturity.push({ label: 'Pipeline units (est.)', value: pipelineUnits.toLocaleString() })
    if (row?.investment_score != null) maturity.push({ label: 'Current signal score', value: `${(Number(row.investment_score) / 10).toFixed(1)}/10` })
    if (row?.gross_yield_pct != null) maturity.push({ label: 'Typical rental yield', value: `${Number(row.gross_yield_pct).toFixed(1)}%` })
    if (appreciation5y != null) maturity.push({ label: '5-year appreciation', value: `${appreciation5y > 0 ? '+' : ''}${appreciation5y}%` })
